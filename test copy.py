    ...
    with open(join(output_dir, args.save_prefix + outfile + 'scores.csv'), 'w') as csvfile:  
        ...
        for img_idx, img in enumerate((test_list)): # Renombrado 'i' a 'img_idx'  
            try:  
                ...
                """ all_patch_inputs = []  
                for patch_iter, patch in enumerate(patches):  
                    patch_input = np.expand_dims(patch, axis=0)  
                    all_patch_inputs.append(patch_input)  
                
                # Convertir la lista de inputs a un solo array NumPy  
                if all_patch_inputs: # Asegurarse de que no esté vacío  
                    batched_patch_inputs = np.concatenate(all_patch_inputs, axis=0)  
                else:  
                    batched_patch_inputs = np.array([]) # Manejar caso de no patches  
                
                # Realizar la predicción en lote  
                if batched_patch_inputs.size > 0:  
                    if args.net.find('caps') != -1:  
                        # Para redes de cápsulas, la salida puede ser una lista de arrays  
                        batched_output_array = eval_model.predict(batched_patch_inputs)  
                        # Extraer las predicciones de los patches del lote  
                        # Asumiendo que batched_output_array[0] es la máscara y tiene forma (num_patches, H, W, 1)  
                        patch_predictions = [batched_output_array[0][k, :, :, 0] for k in range(batched_output_array[0].shape[0])]  
                    else:  
                        # Para otras redes, la salida es un solo array  
                        batched_output_array = eval_model.predict(batched_patch_inputs)  
                        # Extraer las predicciones de los patches del lote  
                        # Asumiendo que batched_output_array tiene forma (num_patches, H, W, 1)  
                        patch_predictions = [batched_output_array[k, :, :, 0] for k in range(batched_output_array.shape[0])]  
                else:  
                    patch_predictions = [] # No hay patches para predecir """
